    ...
    <main className="flex flex-1 flex-col gap-5 p-6">
      ...
      <header>
        <div className="flex flex-wrap items-center gap-2">
          <h1 className="break-words text-2xl font-bold tracking-tight text-on-surface">{eventTitle(event)}</h1>
      ...
      {/* break-words + min-w-0 throughout, not decoration. Google Calendar
          descriptions carry a bare feedback-form URL as one unbreakable token
          — 38 chars on 17,757 events, 111 chars on another 1,021 — and
          whitespace-pre-wrap does not break inside a word. Measured at 375px
          the long one laid out 641px wide and dragged the document to 697px,
          so the phone zoomed out to fit. A grid/flex item's default
          min-width:auto is what let it size the track rather than be clipped
          by max-w-2xl, hence min-w-0 on the containers as well. Same rule the
          schedules list already follows for its selects. */}
      <div className="grid min-w-0 max-w-2xl gap-4">
        <Detail label="Location">
          <p className="break-words font-medium text-on-surface">{event.school?.name ?? event.location_raw ?? "No location"}</p>
          {event.location_raw && event.school && <p className="break-words text-sm text-on-surface-variant">{event.location_raw}</p>}
          {event.school?.address && <p className="break-words text-sm text-on-surface-variant">{event.school.address}</p>}
        </Detail>
        ...
        {event.description && <Detail label="Description"><p className="whitespace-pre-wrap break-words text-sm leading-6 text-on-surface">{event.description}</p></Detail>}
        <Detail label="Organizer"><p className="break-words text-on-surface">{event.organizer_email ?? "Not supplied"}</p></Detail>
...
function Detail({ label, children }: { label: string; children: React.ReactNode }) {
  return <section className="min-w-0 rounded-2xl bg-surface-container p-4 shadow-sm"><h2 className="text-sm font-semibold uppercase tracking-wide text-on-surface-variant">{label}</h2><div className="mt-2">{children}</div></section>;
}
...
function GuestList({ attendees }: { attendees: ScheduleAttendee[] }) {
  ...
  return <ul className="grid gap-2 text-sm">{attendees.map((attendee, index) => <li key={`${attendee.email ?? "guest"}-${index}`} className="flex min-w-0 flex-wrap justify-between gap-2"><span className="min-w-0 break-words text-on-surface">{attendee.displayName || attendee.email || "Guest"}{attendee.email && attendee.displayName ? ` (${attendee.email})` : ""}{attendee.optional ? " · optional" : ""}</span><span className="capitalize text-on-surface-variant">{attendee.responseStatus?.replaceAll("_", " ") ?? "needs action"}</span></li>)}</ul>;
}
