...
> **Feedback rework: Zoho-hosted form + webhook, not an in-app form.** The PRD calls for the class-feedback form to live in Zoho, embedded in the app; Zoho's own webhook (not the teacher's client) closes the attendance session. `close_session_from_zoho()` (service_role-only RPC, replacing the old `clock_out_with_feedback`) and `src/app/api/zoho-feedback/route.ts` (the webhook target, shared-secret authenticated) were exercised live against the real hosted project by simulating a Zoho delivery with `curl` — secret validation, payload validation, the RPC call, and idempotent retry handling all confirmed working end-to-end on a disposable session (cleaned up after). The offline draft path (Dexie-backed local save, prefilled into the Zoho iframe once back online) and the "not configured yet" fallback were verified live in a real browser. The real form URL and prefill (school/teacher/date/class from the class being clocked into) were then added and **confirmed rendering live** — twice: once in an automated check, once by a real teacher account reaching `/clocking` with a real open session, both showing the actual Zoho form load correctly. **What's still unconfirmed**: nobody has filled out and submitted the real form yet, so the actual webhook payload shape/delivery from Zoho (as opposed to the simulated `curl` one) is unverified — see NEXT_STEPS.md. Two real bugs were found and fixed while building this (see DECISIONS.md): the auth proxy was redirecting the webhook to `/login` (fixed by excluding `/api/*` from the proxy matcher), and a hydration mismatch from reading `navigator.onLine` at initial render (fixed by deferring the real value to an effect).
>
...
- `close_session_from_zoho(p_session_id, p_rating, p_summary, p_challenges, p_students_present)` (migration `0010`, replacing `clock_out_with_feedback`) — the ONLY way to close a session. **Not** `SECURITY DEFINER`, **not** granted to `authenticated`/`anon` at all — only `service_role` can call it, since the caller is now the Zoho webhook route handler (no teacher JWT), not the teacher directly. Idempotent: closing an already-closed session is a no-op success (webhooks retry).
